<?php
/**
 * Template Name: Modern Editorial Post with Audio Reader
 * Description: An editorial single post template with a floating scroll progress ring, native text-to-speech, calm mode, and font resizing.
 *
 * @package Podcaster
 * @since 1.0
 */
...
<!-- STEP 1: PRELOADER STYLING & CLEANUP STYLES -->
<style>
  ...
  /* Dynamic Calm Focus Mode transition */
  .calm-mode-transition {
    transition: background-color 0.4s ease, color 0.4s ease;
  }

  /* CSS PRECAUTION: We swap 'overflow-x: hidden' with 'clip' to allow elements with position: sticky to work */
...
<div class="homepage-match-wrapper bg-white font-sans text-gray-800 antialiased min-h-screen calm-mode-transition" id="page-global-container">
	...
	<header class="border-b border-slate-200 py-10 lg:py-16 bg-slate-50 calm-mode-transition" id="editorial-header">
	...
	<div class="max-w-[1440px] mx-auto px-6">
		<div class="grid grid-cols-1 lg:grid-cols-12 gap-0 border-b border-slate-200">
			...
			<!-- Left Column: STICKY FLOATING TOOLS -->
			<aside class="hidden lg:block lg:col-span-2 py-12 pr-8 border-r border-slate-200 relative">
				<div class="sticky top-20 space-y-8 flex flex-col items-center">
					...
					<div class="space-y-3 w-full text-center">
						<p class="text-[10px] uppercase tracking-widest font-bold text-gray-400">Accessibility</p>
						<button id="tts-toggle-btn" onclick="toggleTextToSpeech()" class="w-full bg-[#f1f5f8] border border-blue-100 hover:bg-brandBlue hover:text-white text-brandBlue text-xs font-bold py-2.5 px-3 rounded-lg transition flex items-center justify-center gap-2 shadow-sm">
					...
					<!-- Cool Thing #1: Anxiety-Friendly Calm Mode Dimmer -->
					<div class="space-y-3 w-full text-center">
						<p class="text-[10px] uppercase tracking-widest font-bold text-gray-400">Calm Mode</p>
						<button id="calm-mode-btn" onclick="toggleCalmMode()" class="w-full bg-[#fcfaf2] border border-orange-100 hover:bg-orange-100 text-[#855e42] text-xs font-bold py-2.5 px-3 rounded-lg transition flex items-center justify-center gap-2 shadow-sm">
							<!-- Warm Sunset Icon -->
					...
					<!-- Cool Thing #2: Instant Text Size Resizer -->
					<div class="space-y-3 w-full text-center">
						<p class="text-[10px] uppercase tracking-widest font-bold text-gray-400">Adjust Font Size</p>
						<div class="flex gap-2 justify-center">
							<button onclick="adjustFontSize(-1)" class="w-10 h-10 bg-slate-100 hover:bg-slate-200 text-brandNavy font-extrabold rounded-lg flex items-center justify-center text-sm shadow-sm transition">
								A-
							</button>
							<button onclick="adjustFontSize(1)" class="w-10 h-10 bg-slate-100 hover:bg-slate-200 text-brandNavy font-extrabold rounded-lg flex items-center justify-center text-sm shadow-sm transition">
								A+
			...
			<main class="<?php echo esc_attr( $main_col_span ); ?>" id="editorial-scroll-anchor">
				...
				<div class="block lg:hidden mb-8 bg-[#f1f5f8] p-4 rounded-xl border border-blue-100 space-y-4">
					<div class="flex items-center justify-between">
						...
						<button onclick="toggleTextToSpeech()" class="bg-brandBlue text-white text-xs font-bold py-2 px-4 rounded-lg shadow-sm">
							<span id="tts-label-mobile">Listen</span>
						</button>
					</div>
					<div class="flex items-center justify-between pt-3 border-t border-blue-200/50">
						...
						<div class="flex gap-2">
							...
							<button onclick="adjustFontSize(-1)" class="bg-slate-100 text-xs font-bold py-1 px-2.5 rounded">A-</button>
							<button onclick="adjustFontSize(1)" class="bg-slate-100 text-xs font-bold py-1 px-2.5 rounded">A+</button>
						</div>
					</div>
				</div>
...
<script>
  ...
  function toggleCalmMode() {
    ...
    if (!isCalmModeActive) {
      ...
      if (headerEl) headerEl.style.backgroundColor = '#f4eedf';
      ...
      if (headerEl) headerEl.style.backgroundColor = '#f8fafc';
  ...
  (function() {
    ...
    function initScrollTracker() {
      ...
      var contentAnchor = document.getElementById('article-body-content'); // Focus strictly on body text coordinates [6]
      ...
      window.addEventListener('scroll', function() {
        ...
        // Progress begins when content enters perspective, completing exactly as bottom of text is reached [6]
